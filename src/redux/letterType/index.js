...
export const addlatter_type = createAsyncThunk(
    ...
    async (latter_typeData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/latter-type",
                latter_typeData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updatelatter_type = createAsyncThunk(
    ...
    async ({ id, latter_typeData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/latter-type/${id}`, latter_typeData);
            return response.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addlatter_type.fulfilled, (state, action) => {
                state.loading = false;
                state.latter_type = { ...state.latter_type, data: [action.payload.data, ...state.latter_type.data] };
            ...
            .addCase(updatelatter_type.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.latter_type = { ...state.latter_type, data: [...state.latter_type, action.payload.data] };
                }
                state.success = action.payload.message;
            })
