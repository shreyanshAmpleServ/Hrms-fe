...
export const addstatutory_rates = createAsyncThunk(
    ...
    async (statutory_ratesData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/statutory-rate", statutory_ratesData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to add statutory_rates"
            );
        }
    }
);
...
export const updatestatutory_rates = createAsyncThunk(
    ...
    async ({ id, statutory_ratesData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/statutory-rate/${id}`, statutory_ratesData);
            return response.data;
        } catch (error) {
...
export const deletestatutory_rates = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/statutory-rate/${id}`);
            return {
                data: { id },
                message: response.data.message || "statutory_rates deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to delete statutory_rates"
            );
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addstatutory_rates.fulfilled, (state, action) => {
                state.loading = false;
                state.statutory_rates = { ...state.statutory_rates, data: [action.payload.data, ...state.statutory_rates.data] };
            ...
            .addCase(updatestatutory_rates.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.statutory_rates = { ...state.statutory_rates, data: [...state.statutory_rates, action.payload.data] };
                }
                state.success = action.payload.message;
            })
