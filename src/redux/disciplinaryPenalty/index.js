...
export const fetchdisciplinary_penalty = createAsyncThunk(
    ...
    async (datas, thunkAPI) => {
        try {
            ...
            const response = await apiClient.get("/v1/disciplinary-penalty", { params });
...
export const adddisciplinary_penalty = createAsyncThunk(
    ...
    async (disciplinary_penaltyData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/disciplinary-penalty",
...
export const updatedisciplinary_penalty = createAsyncThunk(
    ...
    async ({ id, disciplinary_penaltyData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/disciplinary-penalty/${id}`, disciplinary_penaltyData);
            return response.data;
        } catch (error) {
...
export const deletedisciplinary_penalty = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/disciplinary-penalty/${id}`);
            return {
                data: { id },
                message: response.data.message || "disciplinary_penalty deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(adddisciplinary_penalty.fulfilled, (state, action) => {
                state.loading = false;
                state.disciplinary_penalty = { ...state.disciplinary_penalty, data: [action.payload.data, ...state.disciplinary_penalty.data] };
            ...
            .addCase(updatedisciplinary_penalty.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.disciplinary_penalty = { ...state.disciplinary_penalty, data: [...state.disciplinary_penalty, action.payload.data] };
                }
                state.success = action.payload.message;
            })
            ...
            .addCase(deletedisciplinary_penalty.fulfilled, (state, action) => {
                ...
                state.disciplinary_penalty = { ...state.disciplinary_penalty, data: filterData }
                state.success = action.payload.message;
            })
