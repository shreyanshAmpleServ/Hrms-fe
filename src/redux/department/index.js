...
export const updatedepartment = createAsyncThunk(
    ...
    async ({ id, departmentData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/department/${id}`, departmentData);
            return response.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(adddepartment.fulfilled, (state, action) => {
                state.loading = false;
                state.department = { ...state.department, data: [action.payload.data, ...state.department.data] };
            ...
            .addCase(updatedepartment.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.department = { ...state.department, data: [...state.department, action.payload.data] };
                }
                state.success = action.payload.message;
            })
