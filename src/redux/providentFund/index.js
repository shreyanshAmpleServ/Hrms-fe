  ...
  async (provident_fundData, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.post("/v1/provident_fund", provident_fundData),
        {
          loading: "Adding Provident Fund...",
          success: "Provident Fund added successfully",
          error: "Failed to add Provident Fund",
        }
      );
      return response.data;
    } catch (error) {
...
export const updateprovident_fund = createAsyncThunk(
  ...
  async ({ id, provident_fundData }, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.put(`/v1/provident_fund/${id}`, provident_fundData),
        {
          loading: "Updating Provident Fund...",
          success: "Provident Fund updated successfully",
          error: "Failed to update Provident Fund",
        }
      );
      return response.data;
    } catch (error) {
      ...
      return thunkAPI.rejectWithValue(
        error.response?.data || "Failed to update Provident Fund"
      );
    }
  }
);
...
export const deleteprovident_fund = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/provident_fund/${id}`),
        {
          loading: "Deleting Provident Fund...",
          success: "Provident Fund deleted successfully",
          error: "Failed to delete Provident Fund",
        }
      );
      return {
        data: { id },
    ...
    builder
      ...
      .addCase(addprovident_fund.fulfilled, (state, action) => {
        ...
        state.provident_fund = {
          ...state.provident_fund,
          data: [action.payload.data, ...state.provident_fund.data],
        };
      ...
      .addCase(updateprovident_fund.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.provident_fund = {
            ...state.provident_fund,
            data: [...state.provident_fund, action.payload.data],
          };
        }
        state.success = action.payload.message;
      })
