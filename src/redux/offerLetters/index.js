    ...
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/offer-letter/${id}`),
        {
          ...
          success: (res) =>
            res.data.message || "offer letter deleted successfully!",
          error: "Failed to delete offer letter",
        }
      );
      return {
        data: { id },
        message: response.data.message || "offer letter deleted successfully",
      };
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data || "Failed to delete offer letter"
      );
    }
  }
);
