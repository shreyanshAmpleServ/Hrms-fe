...
export const fetchwork_life = createAsyncThunk(
    ...
    async (datas, thunkAPI) => {
        try {
            ...
            const response = await apiClient.get("/v1/work-life-event-type", { params });
...
export const addwork_life = createAsyncThunk(
    ...
    async (work_lifeData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/work-life-event-type", work_lifeData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updatework_life = createAsyncThunk(
    ...
    async ({ id, work_lifeData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/work-life-event-type/${id}`, work_lifeData);
            return response.data;
        } catch (error) {
...
export const deletework_life = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/work-life-event-type/${id}`);
            return {
                data: { id },
                message: response.data.message || "work_life deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addwork_life.fulfilled, (state, action) => {
                state.loading = false;
                state.work_life = { ...state.work_life, data: [action.payload.data, ...state.work_life.data] };
            ...
            .addCase(updatework_life.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.work_life = { ...state.work_life, data: [...state.work_life, action.payload.data] };
                }
                state.success = action.payload.message;
            })
