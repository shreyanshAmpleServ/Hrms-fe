  ...
  async (datas, thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(
        error.response?.data || "Failed to fetch rating-scale"
      );
    }
  }
);
  ...
  async (rating_scaleData, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.post("/v1/rating-scale", rating_scaleData),
        {
          loading: "Adding rating scale...",
          success: "Rating scale added successfully",
          error: "Failed to add rating scale",
        }
      );
      return response.data;
    } catch (error) {
...
export const updaterating_scale = createAsyncThunk(
  ...
  async ({ id, rating_scaleData }, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.put(`/v1/rating-scale/${id}`, rating_scaleData),
        {
          loading: "Updating rating scale...",
          success: "Rating scale updated successfully",
          error: "Failed to update rating scale",
        }
      );
      return response.data;
    } catch (error) {
...
export const deleterating_scale = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/rating-scale/${id}`),
        {
          loading: "Deleting rating scale...",
          success: "Rating scale deleted successfully",
          error: "Failed to delete rating scale",
        }
      );
      return {
        data: { id },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addrating_scale.fulfilled, (state, action) => {
        ...
        state.rating_scale = {
          ...state.rating_scale,
          data: [action.payload.data, ...state.rating_scale.data],
        };
      ...
      .addCase(updaterating_scale.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.rating_scale = {
            ...state.rating_scale,
            data: [...state.rating_scale, action.payload.data],
          };
        }
        state.success = action.payload.message;
      })
