...
export const fetchVendors = createAsyncThunk(
  ...
  async (datas, thunkAPI) => {
    try {
      const response = await apiClient.get(`/v1/vendor?search=${datas?.search || ""}&page=${datas?.page || ""}&size=${datas?.size || ""}&startDate=${datas?.startDate?.toISOString() || ""}&endDate=${datas?.endDate?.toISOString() || ""}`);
