...
// salary_structure Slice

// Fetch All salary_structures
...
export const addsalary_structure = createAsyncThunk(
    ...
    async (salary_structureData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/salary-structure", salary_structureData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updatesalary_structure = createAsyncThunk(
    ...
    async ({ id, salary_structureData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/salary-structure/${id}`, salary_structureData);
            return response.data;
        } catch (error) {
...
export const deletesalary_structure = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/salary-structure/${id}`);
            return {
                data: { id },
                message: response.data.message || "salary_structure deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addsalary_structure.fulfilled, (state, action) => {
                state.loading = false;
                state.salary_structure = { ...state.salary_structure, data: [action.payload.data, ...state.salary_structure.data] };
            ...
            .addCase(updatesalary_structure.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.salary_structure = { ...state.salary_structure, data: [...state.salary_structure, action.payload.data] };
                }
                state.success = action.payload.message;
            })
            ...
            .addCase(deletesalary_structure.fulfilled, (state, action) => {
                ...
                state.salary_structure = { ...state.salary_structure, data: filterData }
                state.success = action.payload.message;
            })
