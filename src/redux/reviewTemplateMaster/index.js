  ...
  async (review_templateData, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.post("/v1/review-template", review_templateData),
        {
          loading: "Adding review template...",
          success: "Review template added successfully",
          error: "Failed to add review template",
        }
      );
      return response.data;
    } catch (error) {
...
export const updatereview_template = createAsyncThunk(
  ...
  async ({ id, review_templateData }, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.put(`/v1/review-template/${id}`, review_templateData),
        {
          loading: "Updating review template...",
          success: "Review template updated successfully",
          error: "Failed to update review template",
        }
      );
      return response.data;
    } catch (error) {
...
export const deletereview_template = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/review-template/${id}`),
        {
          loading: "Deleting review template...",
          success: "Review template deleted successfully",
          error: "Failed to delete review template",
        }
      );
      return {
        data: { id },
        message:
          response.data.message || "review_template deleted successfully",
      };
    } catch (error) {
      return thunkAPI.rejectWithValue(
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addreview_template.fulfilled, (state, action) => {
        state.loading = false;
        state.review_template = {
          ...state.review_template,
          data: [action.payload.data, ...state.review_template.data],
        };
      ...
      .addCase(updatereview_template.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.review_template = {
            ...state.review_template,
            data: [...state.review_template, action.payload.data],
          };
        }
        state.success = action.payload.message;
      })
