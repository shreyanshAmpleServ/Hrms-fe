...
export const addpay_component = createAsyncThunk(
    ...
    async (pay_componentData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/pay-component", pay_componentData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to add pay_component"
            );
        }
    }
);
...
export const updatepay_component = createAsyncThunk(
    ...
    async ({ id, pay_componentData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/pay-component/${id}`, pay_componentData);
            return response.data;
        } catch (error) {
...
export const deletepay_component = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/pay-component/${id}`);
            return {
                data: { id },
                message: response.data.message || "pay_component deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to delete pay_component"
            );
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addpay_component.fulfilled, (state, action) => {
                state.loading = false;
                state.pay_component = { ...state.pay_component, data: [action.payload.data, ...state.pay_component.data] };
            ...
            .addCase(updatepay_component.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.pay_component = { ...state.pay_component, data: [...state.pay_component, action.payload.data] };
                }
                state.success = action.payload.message;
            })
