...
export const addemployee_category = createAsyncThunk(
    ...
    async (employee_categoryData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/employee-category", employee_categoryData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to add employee_category"
            );
        }
    }
);
...
export const updateemployee_category = createAsyncThunk(
    ...
    async ({ id, employee_categoryData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/employee-category/${id}`, employee_categoryData);
            return response.data;
        } catch (error) {
...
export const deleteemployee_category = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/employee-category/${id}`);
            return {
                data: { id },
                message: response.data.message || "employee_category deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addemployee_category.fulfilled, (state, action) => {
                state.loading = false;
                state.employee_category = { ...state.employee_category, data: [action.payload.data, ...state.employee_category.data] };
            ...
            .addCase(updateemployee_category.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.employee_category = { ...state.employee_category, data: [...state.employee_category, action.payload.data] };
                }
                state.success = action.payload.message;
            })
            ...
            .addCase(deleteemployee_category.fulfilled, (state, action) => {
                ...
                state.employee_category = { ...state.employee_category, data: filterData }
                state.success = action.payload.message;
            })
