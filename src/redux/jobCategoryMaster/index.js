...
export const addjob_category = createAsyncThunk(
    ...
    async (job_categoryData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/job-category",
                job_categoryData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updatejob_category = createAsyncThunk(
    ...
    async ({ id, job_categoryData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/job-category/${id}`, job_categoryData);
            return response.data;
        } catch (error) {
...
export const deletejob_category = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/job-category/${id}`);
            return {
                data: { id },
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addjob_category.fulfilled, (state, action) => {
                state.loading = false;
                state.job_category = { ...state.job_category, data: [action.payload.data, ...state.job_category.data] };
            ...
            .addCase(updatejob_category.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.job_category = { ...state.job_category, data: [...state.job_category, action.payload.data] };
                }
                state.success = action.payload.message;
            })
