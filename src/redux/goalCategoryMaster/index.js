...
export const addgoal_category = createAsyncThunk(
    ...
    async (goal_categoryData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/goal-category", goal_categoryData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updategoal_category = createAsyncThunk(
    ...
    async ({ id, goal_categoryData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/goal-category/${id}`, goal_categoryData);
            return response.data;
        } catch (error) {
...
export const deletegoal_category = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/goal-category/${id}`);
            return {
                data: { id },
                message: response.data.message || "goal_category deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addgoal_category.fulfilled, (state, action) => {
                state.loading = false;
                state.goal_category = { ...state.goal_category, data: [action.payload.data, ...state.goal_category.data] };
            ...
            .addCase(updategoal_category.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.goal_category = { ...state.goal_category, data: [...state.goal_category, action.payload.data] };
                }
                state.success = action.payload.message;
            })
