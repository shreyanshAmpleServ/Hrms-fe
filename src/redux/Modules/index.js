  ...
  async (moduleData, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.post("/v1/module-related_to", moduleData),
        {
          loading: "Adding Module...",
          success: (res) => res.data.message || "Module added successfully!",
...
export const updateModules = createAsyncThunk(
  ...
  async ({ id, moduleData }, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.put(`/v1/module-related_to/${id}`, moduleData),
        {
          loading: "Updating Module...",
          success: (res) => res.data.message || "Module updated successfully!",
...
export const deleteModules = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/module-related_to/${id}`),
        {
          loading: "Deleting Module...",
          success: (res) => res.data.message || "Module deleted successfully!",
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addModules.fulfilled, (state, action) => {
        ...
        state.modules = {
          ...state.modules,
          data: [action.payload.data, ...state.modules.data],
        };
      ...
      .addCase(updateModules.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.modules = {
            ...state.modules,
            data: [...state.modules, action.payload.data],
          };
        }
