...
export const addsurvey = createAsyncThunk(
    ...
    async (surveyData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/survey",
                surveyData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
...
export const updatesurvey = createAsyncThunk(
    ...
    async ({ id, surveyData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/survey/${id}`, surveyData);
            return response.data;
        } catch (error) {
...
export const deletesurvey = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/survey/${id}`);
            return {
                data: { id },
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addsurvey.fulfilled, (state, action) => {
                state.loading = false;
                state.survey = { ...state.survey, data: [action.payload.data, ...state.survey.data] };
            ...
            .addCase(updatesurvey.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.survey = { ...state.survey, data: [...state.survey, action.payload.data] };
                }
                state.success = action.payload.message;
            })
