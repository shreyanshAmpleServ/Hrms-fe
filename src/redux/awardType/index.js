  ...
  async (award_typeData, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.post("/v1/award-type", award_typeData),
        {
          loading: "Adding award type...",
          success: "Award type added successfully",
          error: "Failed to add award type",
        }
      );
      return response.data;
    } catch (error) {
...
export const updateaward_type = createAsyncThunk(
  ...
  async ({ id, award_typeData }, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.put(`/v1/award-type/${id}`, award_typeData),
        {
          loading: "Updating award type...",
          success: "Award type updated successfully",
          error: "Failed to update award type",
        }
      );
      return response.data;
    } catch (error) {
...
export const deleteaward_type = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await toast.promise(
        apiClient.delete(`/v1/award-type/${id}`),
        {
          loading: "Deleting award type...",
          success: "Award type deleted successfully",
          error: "Failed to delete award type",
        }
      );
      return {
        data: { id },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addaward_type.fulfilled, (state, action) => {
        ...
        state.award_type = {
          ...state.award_type,
          data: [action.payload.data, ...state.award_type.data],
        };
      ...
      .addCase(updateaward_type.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.award_type = {
            ...state.award_type,
            data: [...state.award_type, action.payload.data],
          };
        }
        state.success = action.payload.message;
      })
