...
export const addtax_Regime = createAsyncThunk(
    ...
    async (tax_RegimeData, thunkAPI) => {
        try {
            const response = await apiClient.post("/v1/tax-Regime", tax_RegimeData);
            return response.data;
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to add tax_Regime"
            );
        }
    }
);
...
export const updatetax_Regime = createAsyncThunk(
    ...
    async ({ id, tax_RegimeData }, thunkAPI) => {
        try {
            const response = await apiClient.put(`/v1/tax-Regime/${id}`, tax_RegimeData);
            return response.data;
        } catch (error) {
            ...
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to update tax_Regime"
            );
        }
    }
);
...
export const deletetax_Regime = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            const response = await apiClient.delete(`/v1/tax-Regime/${id}`);
            return {
                data: { id },
                message: response.data.message || "tax_Regime deleted successfully",
            };
        } catch (error) {
            return thunkAPI.rejectWithValue(
                error.response?.data || "Failed to delete tax_Regime"
            );
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(addtax_Regime.fulfilled, (state, action) => {
                state.loading = false;
                state.tax_Regime = { ...state.tax_Regime, data: [action.payload.data, ...state.tax_Regime.data] };
            ...
            .addCase(updatetax_Regime.fulfilled, (state, action) => {
                ...
                if (index !== -1) {
                    ...
                    state.tax_Regime = { ...state.tax_Regime, data: [...state.tax_Regime, action.payload.data] };
                }
                state.success = action.payload.message;
            })
