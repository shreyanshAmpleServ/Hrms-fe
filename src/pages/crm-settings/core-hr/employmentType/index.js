...
const Employee_TypeList = () => {
  ...
  const allPermissions = permissions?.filter(
    (i) => i?.module_name === "Employment Type"
  )?.[0]?.permissions;
  ...
  const columns = [
    {
      title: "Employee Type",
      dataIndex: "type_name",
      render: (text, record) => <Link to={`#`}>{record.type_name}</Link>,
  ...
  const { employmentType, loading, error, success } = useSelector(
  ...
  React.useEffect(() => {
    setPaginationData({
      currentPage: employmentType?.currentPage,
      totalPage: employmentType?.totalPages,
      totalCount: employmentType?.totalCount,
      pageSize: employmentType?.size,
    });
  }, [employmentType]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [searchText, employmentType, columns, sortOrder]);
  ...
  const deleteData = () => {
    if (selected) {
      dispatch(deleteemploymentType(selected.id));
