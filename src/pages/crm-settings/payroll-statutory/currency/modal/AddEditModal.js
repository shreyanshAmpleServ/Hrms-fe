...
const AddEditModal = ({ mode = "add", initialData = null }) => {
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      reset({
        name: initialData.component_name || "",
  ...
  const onSubmit = (data) => {
    ...
    if (mode === "add") {
      ...
      dispatch(
        updateCurrencies({
          id: initialData.id,
          currenciesData: { currencies: data.name, is_active: data.is_active },
        }),
      );
    }
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {mode === "add" ? "Add New Salary Structure" : "Edit Salary"}
            </h5>
            <button
              className="btn-close custom-btn-close border p-1 me-0 text-dark"
              data-bs-dismiss="modal"
              aria-label="Close"
              id="add_Close_currency_modal"
            >
              <i className="ti ti-x" />
            </button>
          </div>
